...
def addUser(username: str=Body(...),nicename:str=Body(...), role:str=Body(...),password: str=Body(...),status: str=Body(...),user : CurrentUser= Depends(CurrentUser)):
  session = Newsession()
  user = session.query(Users).filter_by(name=username).first()
  if not user:
    logger.info(status)
    adduser = Users(name=username, nicename=nicename, role=role, passwd=md5value(password.encode()),status=1 if status else 0)
    session.add(adduser)
    session.commit();
    session.flush()
    data = {"msg": "ok!", "code": 1}
...
def addUser(id: str=Body(...), name: str=Body(...),nicename:str=Body(...), role:str=Body(...),status: str=Body(...),user : CurrentUser= Depends(CurrentUser)):
  session = Newsession()
